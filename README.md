...
| `a02_yfinance_init.py` | 5分足・1時間足・日足データの**初回一括取得** |
| `a03_yfinance_update.py` | 5分足・1時間足・日足データの**定期更新（継ぎ足し）** |
| `a04_yfinance_futures_multi.py` | 先物・為替データの取得（初回自動判定・定期更新対応） |
...
| ファイル名 | 用途 | 必須列 |
|---|---|---|
| `_stock_list.xlsx` | 5分足・1時間足の取得対象銘柄 | `ティッカーコード` |
...
| `_1h.csv` | 1時間足 | 直近2年 | `_stock_list.xlsx` |
...
**2回目以降**はこちらを実行してください。既存CSVに最新データを継ぎ足します。

```bash
python a03_yfinance_update.py
```

- 5分足・1時間足：`_5min.csv` / `_1h.csv` に直近3日分を追記（重複除去あり）
- 日足：`_daily.csv` を直近1年分で上書き更新
- 実行前に5分足・1時間足の自動バックアップを `backups/` ディレクトリに保存

<br><br>

### 4. 先物・為替データの取得（`a04`）

日経先物・原油・金・ドル円の5分足・日足を取得します。CSVの有無により初回/更新を自動判定します。
...
|---|---|---|---|
| `_5min_futures.csv` | 5分足 | 直近60日 | 直近3日 |
| `_daily_futures.csv` | 日足 | 直近2年 | 直近3日 |
...
`_5min.csv` / `_1h.csv` / `_daily.csv` の品質を確認します。
...
定期実行（毎日・毎週など）
    ↓
python a03_yfinance_update.py         # 時系列データの継ぎ足し
python a04_yfinance_futures_multi.py  # 先物・為替の継ぎ足し
