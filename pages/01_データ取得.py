...
_JOB_SAVE_PATHS: dict[str, str] = {
    ...
    "OHLCV（株価）":       "data/prices/stocks/",
    "OHLCV（マクロ）":     "data/prices/macro/",
    "財務（EDINET DB）":   "data/financials/",
    "決算短信（JSON）":    "data/statements/",
    "適時開示（TDnet）":   "data/news/tdnet/",
    "カレンダー（J-Quants）": "data/news/kessan/",
    "ニュース（RSS）":     "data/news/rss/",
    "有報インデックス（EDINET）": "data/edinet_index.parquet",
    "有報 XBRL（EDINET）":       "data/yuho_zip/",
}
...
def _edinet_idx_coverage() -> str:
    ...
    try:
        ...
        return f"{dates[0]} 〜 {dates[-1]}（{len(df)} 件）"
    except Exception:
        return "確認中…"


@st.cache_data(ttl=60, show_spinner=False)
def _yuho_coverage() -> str:
    if not YUHO_ZIP.exists():
        return "—（未取得）"
    zips = list(YUHO_ZIP.rglob("*.zip"))
    if not zips:
        return "—（未取得）"
    jsons = list(YUHO.rglob("*.json")) if YUHO.exists() else []
    return f"ZIP {len(zips)} 件 / JSON {len(jsons)} 件"
...
# ── 一括取得（メイン） ──────────────────────────────────────
_btn_c1, _btn_c2, _btn_c3 = st.columns([3,0.5,0.5])
with _btn_c1:
    st.html('<div style="height: 8px;"></div>')
    daily_clicked = st.button(
        "📥 各データを一括取得", width="stretch", type="primary",
    )
with _btn_c2:
    _fivemin_days = st.number_input(
        "5分足日数", min_value=1, max_value=59, value=3, step=1,
    )
with _btn_c3:
    _daily_years = st.number_input(
        "日足年数", min_value=1, max_value=10, value=3, step=1,
    )

with st.expander("各データを個別に取得"):
    st.html('<div style="height: 16px;"></div>')
    space1, _i1, space2, _i2, space3 = st.columns([0.1,1,0.1,1,0.1])
    with _i1:
        _i_ohlcv_clicked = st.button("📅 OHLCVデータ（株価）", width="stretch",
            help="yfinance で個別銘柄の日足・5分足を取得")
    with _i2:
        _i_macro_clicked = st.button("📅 OHLCVデータ（マクロ指標）", width="stretch",
            help="yfinance でマクロ指標の日足を取得")

    space1, _i3, space2, _i4, space3 = st.columns([0.1,1,0.1,1,0.1])
    with _i3:
        _i_edinet_idx_clicked = st.button("📅 有報インデックス（EDINET）", width="stretch",
            help=f"金融庁 EDINET API で本日の有価証券報告書インデックスを取得。取得済み: {_edinet_idx_coverage()}")
    with _i4:
        _i_edinet_xbrl_clicked = st.button("📅 有報 XBRL（EDINET）", width="stretch",
            help=f"Phase 1 対象 13 銘柄の有報 XBRL ZIP をダウンロード。取得済み: {_yuho_coverage()}")

    space1, _i5, space2, _i6, space3 = st.columns([0.1,1,0.1,1,0.1])
    with _i5:
        _i_json_clicked = st.button("📅 決算短信JSON（TDne）", width="stretch",
            help=f"TDnet から {_LATEST_BD_STR} の決算短信 XBRL ZIP を取得・JSON 変換")
    with _i6:
        _i_fin_clicked = st.button("📅 財務データ（EDINET DB）", width="stretch",
            help="EDINET DB API で財務データを取得")

    space1, _i7, space2, _i8, space3 = st.columns([0.1,1,0.1,1,0.1])
    with _i7:
        _i_tdnet_clicked = st.button("📅 適時開示（TDnet）", width="stretch",
            help=f"TDnet から {_LATEST_BD_STR} の適時開示一覧を取得")
    with _i8:
        _i_kessan_clicked = st.button("📅 決算短信カレンダー（J-Quants）", width="stretch",
            help=f"J-Quants API で {_LATEST_BD_STR} の決算短信発表実績を取得")

    space1, _i9, space2, _i10, space3 = st.columns([0.1,1,0.1,1,0.1])
    with _i9:
        _i_rss_clicked = st.button("📅 ニュース（RSS）", width="stretch",
            help="RSS フィードから本日のニュースを取得")
    with _i10:
        pass

    if _i_ohlcv_clicked:
        with st.spinner("OHLCV（株価）取得中…"):
            res = _run_parallel({"OHLCV（株価）": [
                "python", "collectors/fetch_prices_stocks.py",
                "--fivemin-days", str(int(_fivemin_days)),
                "--daily-years",  str(int(_daily_years)),
            ]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("OHLCV（株価）取得", res)

    if _i_macro_clicked:
        with st.spinner("OHLCV（マクロ）取得中…"):
            res = _run_parallel({"OHLCV（マクロ）": ["python", "collectors/fetch_prices_macro.py"]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("OHLCV（マクロ）取得", res)

    if _i_fin_clicked:
        with st.spinner("財務データ取得中…"):
            res = _run_parallel({"財務（EDINET DB）": ["python", "collectors/fetch_financials.py"]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("財務データ取得", res)

    if _i_json_clicked:
        with st.spinner("決算短信 JSON 変換中…"):
            res = _run_parallel({"決算短信（JSON）": [
                "python", "collectors/fetch_statements.py", "--date", _LATEST_BD_STR,
            ]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("決算短信 JSON 変換", res)

    if _i_tdnet_clicked:
        with st.spinner("適時開示取得中…"):
            res = _run_parallel({"適時開示（TDnet）": [
                "python", "collectors/fetch_tdnet.py", "--date", _LATEST_BD_STR,
            ]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("適時開示取得", res)

    if _i_kessan_clicked:
        with st.spinner("決算短信カレンダー取得中…"):
            res = _run_parallel({"カレンダー（J-Quants）": [
                "python", "collectors/fetch_kessan_calendar.py",
                "--start-date", _LATEST_BD_STR, "--end-date", _LATEST_BD_STR,
            ]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("決算短信カレンダー取得", res)

    if _i_rss_clicked:
        with st.spinner("ニュース取得中…"):
            res = _run_parallel({"ニュース（RSS）": ["python", "collectors/fetch_rss.py"]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("ニュース取得", res)

    if _i_edinet_idx_clicked:
        with st.spinner("有報インデックス取得中…"):
            res = _run_parallel({"有報インデックス（EDINET）": [
                "python", "collectors/fetch_edinet_index.py", "--date", _LATEST_BD_STR,
            ]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("有報インデックス取得", res)

    if _i_edinet_xbrl_clicked:
        with st.spinner("有報 XBRL ダウンロード中（時間がかかります）…"):
            res = _run_parallel({"有報 XBRL（EDINET）": [
                "python", "collectors/fetch_edinet_xbrl.py",
            ]})
        st.cache_data.clear()
        st.session_state["_fetch_result"] = ("有報 XBRL ダウンロード", res)
    st.html('<div style="height: 16px;"></div>')
...
if daily_clicked:
    with st.spinner("日次データを並列実行中…"):
        results = _run_parallel({
            ...
            "④TDnet":        ["python", "collectors/fetch_tdnet.py", "--date", _LATEST_BD_STR],
            "⑤RSS":          ["python", "collectors/fetch_rss.py"],
            "⑥決算短信XBRL": ["python", "collectors/fetch_statements.py", "--date", _LATEST_BD_STR],
            "決算発表予定":   ["python", "collectors/fetch_kessan_calendar.py", "--mode", "future"],
        })
    st.cache_data.clear()
...
with st.expander("📝 メモ（取得されるデータについて）"):
    st.markdown("""
...
|  | データ | 取得元 |
|:--|:--|:--|
| 1 | OHLCVデータ（銘柄・マクロ指標） | yfinance |
| 2 | 有価証券報告書（XBRL） | 金融庁 EDINET API v2 |
| 3 | 決算短信（XBRL・JSON） | TDnet XBRL |
| 4 | 財務データ | EDINET DB API |
| 5 | 適時開示 | TDnet |
| 6 | 決算短信発表カレンダー | J-Quants API |
| 7 | ニュース | RSS |
...
##### 📑 有報インデックス（EDINET）／📦 有報 XBRL（EDINET）

保存先: `data/edinet_index.parquet`（インデックス）、`data/yuho_zip/{edinet_code}/{doc_id}.zip`（ZIP）

金融庁 EDINET API v2 から有価証券報告書（docTypeCode: 120/130）を 2 ステップで取得します。

**ステップ①　有報インデックス取得（毎営業日）**

EDINET API はその日に提出された書類の一覧を日付ごとに返します。この一覧を取得して `edinet_index.parquet` に追記するのが「有報インデックス取得」です。

```
例: 2026-05-09 に取得した場合
  5020（ENEOS）  → docID: S100XXXX, 有価証券報告書, 2025-03-期
  8001（伊藤忠） → docID: S100YYYY, 有価証券報告書, 2025-03-期  ...
```

この docID が ZIP ダウンロードのキーになるため、②の前提として毎営業日の実行を推奨します。

**ステップ②　有報 XBRL 取得（随時）**

インデックスを参照して Phase 1 対象 13 銘柄（石油元売・総合商社・資源）の XBRL ZIP をダウンロードします。有報の提出は 3〜6 月に集中するため、年 1〜2 回まとめて実行する運用が現実的です。

設定: `.env` に `EDINET_API_KEY` を記載（`api.edinet-fsa.go.jp` で登録）。
...
with st.expander("過去データを一括取得して補正"):
        ...
        if not _hist_start or not _hist_end:
            ...
            if _hist_tdnet_clicked:
                with st.spinner(f"TDnet 一括取得中（{_s} 〜 {_e}）…"):
                    results = _run_parallel({
                        "TDnet一括": [
                            "python", "collectors/fetch_tdnet.py",
