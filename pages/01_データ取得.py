...
from config.paths import PRICES_STOCKS_DAILY, PRICES_MACRO_DAILY, NEWS_KESSAN_SCHEDULE
...
def _kessan_schedule_coverage() -> str:
    p = NEWS_KESSAN_SCHEDULE / "latest.csv"
    if not p.exists():
        return "未取得"
    try:
        import pandas as pd
        df = pd.read_csv(p)
        fetched = df["fetched_at"].iloc[0] if "fetched_at" in df.columns else "不明"
        return f"{len(df)} 件（取得: {fetched}）"
    except Exception:
        return "読み込みエラー"
...
with col2:
    ...
    if st.button("📅 決算発表予定（J-Quants）", width="stretch",
                 help=f"J-Quants API で翌営業日の決算発表予定を取得（3月・9月決算企業のみ）。APIキー（JQUANTS_API_KEY）が必要。\n現在: {_kessan_schedule_coverage()}\n保存先: data/news/kessan_schedule/latest.csv"):
        with st.spinner("決算発表予定を取得中…"):
            rc, out = _run(["python", "collectors/fetch_kessan_calendar.py", "--mode", "future"])
        _show_result("決算発表予定（J-Quants）", rc, out)
...
**決算発表予定（J-Quants API v2）**

保存先: `data/news/kessan_schedule/latest.csv`（取得のたびに上書き）

J-Quants API v2 の `/equities/earnings-calendar` から翌営業日の決算発表予定を取得。
`.env` に `JQUANTS_API_KEY` が必要（J-Quantsダッシュボードで発行）。
...
※ 3月・9月決算企業のみ対象。翌営業日分のみ提供（将来複数日分は取得不可）。
""")