...
def to_ticker(code):
    """
    銘柄コードに「.T」を付与してTickerシンボルに変換する。
    例: 7203 → "7203.T"、7203.0 → "7203.T"
    """
    s = str(code).strip()
...
def load_tickers(interval: str) -> list:
    ...
    try:
        if interval == "1d":
            ...
            tickers = [to_ticker(c) for c in df[TOPIX_CODE_COL].dropna().unique()]
